...
export class AbstractAnneeScolaireModal {
  ...
  getFormTemplate() {
    return `
      <div class="form-control">
        ...
        <input type="text" name="libelle" class="input input-bordered input-primary" required maxlength="20" placeholder="Ex: 2023-2024">
        <div data-error="libelle" class="text-error text-sm mt-1 hidden"></div>
        <div class="text-xs text-gray-500 mt-1">Format: AAAA-AAAA (ex: 2023-2024)</div>
      </div>
    `;
  }
  ...
  setupModal() {
    this.modal = new Modal({
      title: this.config.title || "Année scolaire",
      content: this.form,
      size: "md",
    ...
    this.fields = {
      libelle: {
        ...
        validator: async (v) => {
          ...
          if (!validators.maxLength(v, 20)) return "Maximum 20 caractères";
          if (!/^\d{4}-\d{4}$/.test(v))
            return "Format invalide (ex: 2023-2024)";

          const currentLibelle = this.config?.anneeScolaire?.libelle || null;
          if (
            currentLibelle &&
