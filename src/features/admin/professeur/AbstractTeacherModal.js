...
export class AbstractTeacherModal {
    ...
    return `
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="form-control">
          <label class="label">
            <span class="label-text flex items-center gap-2">
              Spécialité <span class="text-error">*</span>
            </span>
          </label>
          <input type="text" name="specialite" class="input input-bordered input-primary" required maxlength="100">
          <div data-error="specialite" class="text-error text-sm mt-1 hidden"></div>
        </div>
        ...
        <div class="form-control">
          <label class="label">
            <span class="label-text flex items-center gap-2">
              Date d'embauche <span class="text-error">*</span>
            </span>
          </label>
          <input type="date" name="date_embauche" class="input input-bordered input-primary" required>
          <div data-error="date_embauche" class="text-error text-sm mt-1 hidden"></div>
        </div>
      </div>
    `;
  }
  ...
  setupModal() {
    this.modal = new Modal({
      title: this.config.title || "Professeur",
      content: this.form,
      size: "xl",
  ...
  setupValidation() {
    this.fields = {
      ...
      date_embauche: {
        ...
        error: "",
        validator: (v) => {
          if (!validators.required(v)) return "La date d'embauche est requise";
      ...
      email: {
        ...
        validator: async (v) => {
          ...
          const currentEmail = this.config?.teacher?.user?.email || null;
      ...
      telephone: {
        ...
        validator: async (v) => {
          ...
          const currentPhone = this.config?.teacher?.user?.telephone || null;
          if (currentPhone === v) {
            return true;
          }
      ...
      specialite: {
        value: "",
        error: "",
        validator: (v) => validators.required(v) || "La spécialité est requise",
      },
      adresse: {
        value: "",
        error: "",
  ...
  getFormData() {
   const formData = new FormData(this.form);
    ...
    formData.append("date_embauche", this.form.querySelector('[name="date_embauche"]').value);
    formData.append(
      "telephone",
