export class ClassCard {
  constructor(config) {
    this.config = {
      data: [],
      itemsPerPage: 8,
      containerId: "class-cards",
      actions: null,
      onAction: null,
      emptyMessage: "Aucune classe disponible",
      ...config,
    };
  ...
  renderCards() {
    ...
    itemsToShow.forEach((classe) => {
      this.grid.appendChild(this.createClassCard(classe));
    });

    this.updatePagination();
  }

  createClassCard(classe) {
    ...
    header.innerHTML = `
      <div class="w-12 h-12 rounded-full bg-green-100 flex items-center justify-center mr-3">
        <i class="ri-door-line text-green-600 text-xl"></i>
      </div>
      <div>
        <h3 class="text-lg font-semibold">${classe.nom}</h3>
        <p class="text-sm text-gray-500">${classe.niveau?.libelle || "N/A"} • ${
      classe.anneeScolaire?.libelle || "N/A"
    }</p>
      </div>
    `;
    ...
    capacity.className = "badge badge-primary badge-sm self-start mb-2";
    capacity.innerHTML = `<i class="ri-user-line mr-1"></i> ${
      classe.capacite_max || "∞"
    }`;
    ...
    status.className = `badge badge-${
      classe.statut === "actif" ? "success" : "warning"
    } badge-sm self-start mb-4`;
    status.textContent = classe.statut === "actif" ? "Actif" : "Inactif";
    ...
    if (this.config.actions) {
      cardActions.appendChild(this.renderActions(classe));
    }
  ...
  updatePagination() {
    ...
    this.paginationInfo.textContent = `Page ${this.currentPage} sur ${totalPages} • ${this.config.data.length} classes`;
  }
