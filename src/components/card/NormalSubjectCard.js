export class NormalSubjectCard {
  constructor(config) {
    this.config = {
      data: [],
      itemsPerPage: 8,
  ...
  createSubjectCard(subject) {
    ...
    header.innerHTML = `
      ...
      <div>
        <h3 class="text-lg font-semibold">${subject.nom}</h3>
        <p class="text-sm text-gray-500">${subject.niveau.libelle || "N/A"}</p>
      </div>
    `;
    ...
    coefficient.textContent = `Coeff: ${subject.coefficient}`;
