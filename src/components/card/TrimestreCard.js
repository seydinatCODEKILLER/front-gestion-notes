export class TrimestreCard {
  constructor(config) {
    this.config = {
      data: [],
      itemsPerPage: 8,
      containerId: "trimestre-cards",
      actions: null,
      onAction: null,
      emptyMessage: "Aucun trimestre disponible",
      ...config,
    };
  ...
  renderCards() {
    ...
    itemsToShow.forEach((trimestre) => {
      this.grid.appendChild(this.createTrimestreCard(trimestre));
    });

    this.updatePagination();
  }

  createTrimestreCard(trimestre) {
    ...
    header.innerHTML = `
      <div class="w-12 h-12 rounded-full bg-purple-100 flex items-center justify-center mr-3">
        <i class="ri-calendar-2-line text-purple-600 text-xl"></i>
      </div>
      <div>
        <h3 class="text-lg font-semibold">${trimestre.libelle}</h3>
        <p class="text-sm text-gray-500">${
          trimestre.annee_scolaire?.libelle || "N/A"
        }</p>
      </div>
    `;
    ...
    status.className = `badge badge-${
      trimestre.statut === "actif" ? "success" : "warning"
    } badge-sm self-start mb-4`;
    status.textContent = trimestre.statut === "actif" ? "Actif" : "Inactif";
    ...
    if (this.config.actions) {
      cardActions.appendChild(this.renderActions(trimestre));
    }
  ...
  updatePagination() {
    ...
    this.paginationInfo.textContent = `Page ${this.currentPage} sur ${totalPages} • ${this.config.data.length} trimestres`;
  }
