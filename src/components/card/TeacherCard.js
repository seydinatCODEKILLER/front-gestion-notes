export class TeacherCard {
  ...
  renderCards() {
    ...
    itemsToShow.forEach((teacher) => {
      this.grid.appendChild(this.createTeacherCard(teacher));
    });
  ...
  createTeacherCard(teacher) {
    ...
    header.innerHTML = `
      <div class="w-12 h-12 rounded-full bg-blue-100 flex items-center justify-center mr-3 overflow-hidden">
        ${
          user.avatar
      ...
      <div>
        ...
        <p class="text-sm text-gray-500">${
          teacher.specialite || "Non spécifié"
        }</p>
      </div>
    `;
    ...
    contact.className = "space-y-1 mb-4 text-sm";
    contact.innerHTML = `
      ...
      ${
        teacher.date_embauche
          ? `
        <div class="flex items-center text-gray-600">
          <i class="ri-calendar-event-line mr-2"></i>
          <span>Embauche: ${new Date(
            teacher.date_embauche
    ...
    if (this.config.actions) {
      cardActions.appendChild(this.renderActions(teacher));
    }
  ...
  updatePagination() {
    ...
    this.paginationInfo.textContent = `Page ${this.currentPage} sur ${totalPages} • ${this.config.data.length} professeurs`;
  }
