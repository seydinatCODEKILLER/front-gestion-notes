export class NiveauCard {
  constructor(config) {
    this.config = {
      data: [],
      itemsPerPage: 8,
      containerId: "niveau-cards",
      actions: null,
      onAction: null,
      emptyMessage: "Aucun niveau disponible",
      ...config,
    };
  ...
  renderCards() {
    ...
    itemsToShow.forEach((niveau) => {
      this.grid.appendChild(this.createNiveauCard(niveau));
    });

    this.updatePagination();
  }

  createNiveauCard(niveau) {
    ...
    header.innerHTML = `
      ...
      <h3 class="text-lg font-semibold">${niveau.libelle}</h3>
    `;

    // Status badge
    const status = document.createElement("div");
    status.className = `badge badge-${
      niveau.statut === "actif" ? "success" : "warning"
    } badge-sm self-start mb-4`;
    status.textContent = niveau.statut === "actif" ? "Actif" : "Inactif";
    ...
    if (this.config.actions) {
      cardActions.appendChild(this.renderActions(niveau));
    }
  ...
  updatePagination() {
    ...
    this.paginationInfo.textContent = `Page ${this.currentPage} sur ${totalPages} • ${this.config.data.length} niveaux`;
  }
